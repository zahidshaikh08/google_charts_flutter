...
const styleSegmentsKey = AttributeKey<List<_LineRendererElement<Object>>>('LineRenderer.styleSegments');
...
class LineRenderer<D> extends BaseCartesianRenderer<D> {
  ...
  factory LineRenderer({String? rendererId, LineRendererConfig<D>? config}) {
    return LineRenderer._internal(
        rendererId: rendererId ?? 'line', config: config ?? LineRendererConfig<D>());
  }

  LineRenderer._internal({required String rendererId, required this.config})
      : _pointRenderer = PointRenderer<D>(config: PointRendererConfig<D>(radiusPx: config.radiusPx)),
        super(
  ...
  void configureSeries(List<MutableSeries<D>> seriesList) {
    ...
    seriesList.forEach((series) {
      ...
      series.areaColorFn ??= (int? index) {
        ...
        return Color(r: color.r, g: color.g, b: color.b, a: (color.a * config.areaOpacity).round());
      };
    });
  ...
  void preprocessSeries(List<MutableSeries<D>> seriesList) {
    ...
    _hasMeasureBounds =
        seriesList.any((series) => series.measureUpperBoundFn != null && series.measureLowerBoundFn != null);
    ...
    seriesList.forEach((MutableSeries<D> series) {
      ...
      for (var index = 0; index < series.data.length; index++) {
        ...
        final strokeWidthPx = strokeWidthPxFn?.call(index)?.toDouble() ?? config.strokeWidthPx;
    ...
    if (config.stacked && seriesList.isNotEmpty) {
      ...
      for (var i = 0; i < seriesList.length; i++) {
        ...
        final measureOffsetFn = _createStackedMeasureOffsetFunction(series, curOffsets, nextOffsets);
  ...
  AccessorFn<num?> _createStackedMeasureOffsetFunction(
      MutableSeries<D> series, Map<D, num?> curOffsets, Map<D, num> nextOffsets) {
    final domainFn = series.domainFn;
    final measureFn = series.measureFn;
  ...
  void update(List<ImmutableSeries<D>> seriesList, bool isAnimatingThisDraw) {
    ...
    seriesList.forEach((ImmutableSeries<D> series) {
      ...
      styleSegments.forEach((styleSegment) {
        ...
        var animatingElements = elementsList.firstWhereOrNull((elements) => elements.styleKey == styleKey);
        ...
        if (animatingElements != null) {
          ...
          final lineAndArea = _createLineAndAreaElements(series, styleSegment as _LineRendererElement<D>,
              stackIndex > 0 ? previousInitialPointList[stackIndex - 1] : null, true);
          ...
          final boundsElementList = lineAndArea[3] as List<_AreaRendererElement<D>>;
          ...
          for (var index = 0; index < lineElementList.length; index++) {
            animatingLines.add(
                _AnimatedLine<D>(key: lineElementList[index].styleKey, overlaySeries: series.overlaySeries)
          ...
          if (config.includeArea) {
            ...
            for (var index = 0; index < areaElementList.length; index++) {
              animatingAreas.add(
                  _AnimatedArea<D>(key: areaElementList[index].styleKey, overlaySeries: series.overlaySeries)
          ...
          if (_hasMeasureBounds) {
            ...
            for (var index = 0; index < boundsElementList.length; index++) {
              animatingBounds.add(_AnimatedArea<D>(
                  key: boundsElementList[index].styleKey, overlaySeries: series.overlaySeries)
        ...
        for (var index = 0; index < lineElementList.length; index++) {
          ...
          if (index >= animatingElements.lines.length) {
            animatingElements.lines
                .add(_AnimatedLine<D>(key: lineElement.styleKey, overlaySeries: series.overlaySeries));
          }
        ...
        if (config.includeArea) {
          for (var index = 0; index < areaElementList.length; index++) {
            ...
            if (index >= animatingElements.areas!.length) {
              animatingElements.areas!
                  .add(_AnimatedArea<D>(key: areaElement.styleKey, overlaySeries: series.overlaySeries));
            }
        ...
        if (_hasMeasureBounds) {
          for (var index = 0; index < boundsElementList.length; index++) {
            ...
            if (index >= animatingElements.bounds!.length) {
              animatingElements.bounds!
                  .add(_AnimatedArea<D>(key: boundElement.styleKey, overlaySeries: series.overlaySeries));
            }
  ...
  List<Object> _createLineAndAreaElements(ImmutableSeries<D> series, _LineRendererElement<D> styleSegment,
      List<_DatumPoint<D>>? previousPointList, bool initializeFromZero) {
  ...
  /// values of 0, or using series data. This should be true when calculating
  /// point positions to animate in from the measure axis.
  List<_DatumPoint<D>> _createPointListForSeries(ImmutableSeries<D> series, bool initializeFromZero) {
    final domainAxis = series.getAttr(domainAxisKey) as ImmutableAxis<D>;
    final domainFn = series.domainFn;
    final measureAxis = series.getAttr(measureAxisKey) as ImmutableAxis<num>;
    final measureFn = series.measureFn;
    ...
    for (var index = 0; index < series.data.length; index++) {
      ...
      pointList.add(_getPoint(datum, domainFn(index), series, domainAxis, measure, measureOffset, measureAxis,
          index: index));
    }
  ...
  ///
  /// [series] the series that this line represents.
  List<List<List<_DatumPoint<D>>>> _createLineAndAreaSegmentsForSeries(List<_DatumPoint<D>> pointList,
      List<_DatumPoint<D>>? previousPointList, ImmutableSeries<D> series, bool initializeFromZero) {
    ...
    final seriesHasMeasureBounds = series.measureUpperBoundFn != null && series.measureLowerBoundFn != null;
    ...
    for (var index = 0; index < pointList.length; index++) {
      ...
      if (point.y == null) {
        ...
        lineSegments.add(_createLineSegment(startPointIndex, endPointIndex!, pointList));

        // Isolated data points are handled by the line painter. Do not add an
        // area segment for them.
        if (startPointIndex != endPointIndex) {
          if (config.includeArea) {
            areaSegments.add(_createAreaSegment(
                startPointIndex, endPointIndex, pointList, previousPointList, series, initializeFromZero));
          }
          if (seriesHasMeasureBounds) {
            boundsSegments.add(_createBoundsSegment(
                pointList.sublist(startPointIndex, endPointIndex + 1), series, initializeFromZero));
          }
        }
  ...
  ///
  /// [pointList] list of all points in the line.
  List<_DatumPoint<D>> _createLineSegment(int start, int end, List<_DatumPoint<D>> pointList) =>
  ...
  ///
  /// [series] the series that this line represents.
  List<_DatumPoint<D>> _createAreaSegment(int start, int end, List<_DatumPoint<D>> pointList,
      List<_DatumPoint<D>>? previousPointList, ImmutableSeries<D> series, bool initializeFromZero) {
    final domainAxis = series.getAttr(domainAxisKey) as ImmutableAxis<D>;
    final domainFn = series.domainFn;
    final measureAxis = series.getAttr(measureAxisKey) as ImmutableAxis<num>;
    ...
    if (!config.stacked || previousPointList == null) {
      ...
      areaPointList.add(_getPoint(null, domainFn(end), series, domainAxis, 0.0, 0.0, measureAxis));

      areaPointList.add(_getPoint(null, domainFn(start), series, domainAxis, 0.0, 0.0, measureAxis));
    } else {
  ...
  List<_DatumPoint<D>> _createBoundsSegment(
      List<_DatumPoint<D>> pointList, ImmutableSeries<D> series, bool initializeFromZero) {
    final measureAxis = series.getAttr(measureAxisKey) as ImmutableAxis<num>;
    final areaPointList = <_DatumPoint<D>>[];
    ...
    areaPointList.addAll(pointList.map((datumPoint) => _DatumPoint.from(
        ...
        initializeFromZero
            ? datumPoint.y
            : measureAxis.getLocation((series.measureUpperBoundFn!(datumPoint.index) ?? 0) +
                series.measureOffsetFn!(datumPoint.index)!))));

    // Add all points for lower bounds, in reverse order.
    areaPointList.addAll(pointList.reversed.map((datumPoint) => _DatumPoint.from(
        datumPoint,
        datumPoint.x,
        initializeFromZero
            ? datumPoint.y
            : measureAxis.getLocation((series.measureLowerBoundFn!(datumPoint.index) ?? 0) +
                series.measureOffsetFn!(datumPoint.index)!))));
  ...
  _Range<num> _createPositionExtent(ImmutableSeries<D> series, _LineRendererElement<D> details) {
    final domainAxis = series.getAttr(domainAxisKey) as ImmutableAxis<D>;
    ...
    final startPosition = domainAxis.getLocation(details.domainExtent.start) ?? drawBounds!.left.toDouble();

    // Clamp end position to the end of the draw area if it is outside the
    // domain viewport range.
    final endPosition = domainAxis.getLocation(details.domainExtent.end) ?? drawBounds!.right.toDouble();
  ...
  void paint(ChartCanvas canvas, double animationPercent) {
    ...
    if (animationPercent == 1.0) {
      ...
      _seriesLineMap.forEach((String key, List<_AnimatedElements<D>> elements) {
        elements.removeWhere((_AnimatedElements<D> element) => element.animatingOut);
    ...
    _seriesLineMap.forEach((String key, List<_AnimatedElements<D>> elements) {
      if (config.includeArea) {
        elements
            .map<List<_AnimatedArea<D>>>((_AnimatedElements<D> animatingElement) => animatingElement.areas!)
            .expand<_AnimatedArea<D>>((List<_AnimatedArea<D>> areas) => areas)
            .map<_AreaRendererElement<D>>(
                (_AnimatedArea<D> animatingArea) => animatingArea.getCurrentArea(animationPercent))
            .forEach((area) {
          if (area != null) {
            canvas.drawPolygon(
      ...
      if (_hasMeasureBounds) {
        elements
            .map<List<_AnimatedArea<D>>>((_AnimatedElements<D> animatingElement) => animatingElement.bounds!)
            .expand<_AnimatedArea<D>>((List<_AnimatedArea<D>> bounds) => bounds)
            .map<_AreaRendererElement<D>>(
                (_AnimatedArea<D> animatingBounds) => animatingBounds.getCurrentArea(animationPercent))
            .forEach((bound) {
          if (bound != null) {
            canvas.drawPolygon(
      ...
      if (config.includeLine) {
        elements
            .map<List<_AnimatedLine<D>>>((_AnimatedElements<D> animatingElement) => animatingElement.lines)
            .expand<_AnimatedLine<D>>((List<_AnimatedLine<D>> lines) => lines)
            .map<_LineRendererElement<D>>(
                (_AnimatedLine<D> animatingLine) => animatingLine.getCurrentLine(animationPercent))
            .forEach((line) {
          if (line != null) {
            canvas.drawLine(
  ...
  Rectangle<num> _getClipBoundsForExtent(_Range<num> extent) {
    ...
    return Rectangle<num>(left, drawBounds.top - drawBoundTopExtensionPx, right - left,
        drawBounds.height + drawBoundTopExtensionPx + drawBoundBottomExtensionPx);
  }

  bool get isRtl => _chart?.context.isRtl ?? false;

  _DatumPoint<D> _getPoint(dynamic datum, D? domainValue, ImmutableSeries<D> series,
      ImmutableAxis<D> domainAxis, num? measureValue, num? measureOffsetValue, ImmutableAxis<num> measureAxis,
      {int? index}) {
        ...
        for (final p in segment.allPoints) {
          ...
          if (byDomain) {
            if ((domainDistance < nearestDomainDistance) ||
                ((domainDistance == nearestDomainDistance) && (measureDistance < nearestMeasureDistance))) {
              nearestPoint = p;
  ...
  bool _isPointBelowSeries(
      Point<double> chartPoint, _DatumPoint<D> nearestPoint, List<_DatumPoint<D>> allPointsForSeries) {
    _DatumPoint<D>? leftPoint;
    _DatumPoint<D>? rightPoint;
    var nearestPointIdx = allPointsForSeries.indexWhere((p) => p == nearestPoint);
    if (chartPoint.x < nearestPoint.x!) {
      leftPoint = nearestPointIdx > 0 ? allPointsForSeries[nearestPointIdx - 1] : null;
      rightPoint = nearestPoint;
    } else {
      leftPoint = nearestPoint;
      rightPoint =
          nearestPointIdx < allPointsForSeries.length - 1 ? allPointsForSeries[nearestPointIdx + 1] : null;
    }
    var limit = chartPoint.y;
    if (leftPoint != null && leftPoint.y != null && rightPoint != null && rightPoint.y != null) {
  ...
  DatumDetails<D> addPositionToDetailsForSeriesDatum(DatumDetails<D> details, SeriesDatum<D> seriesDatum) {
    ...
    final point = _getPoint(seriesDatum.datum, details.domain, series, domainAxis, details.measure,
        details.measureOffset, measureAxis);
...
class _LineRendererElement<D> {
  ...
  void updateAnimationPercent(
      _LineRendererElement<D> previous, _LineRendererElement<D> target, double animationPercent) {
    ...
    for (pointIndex = 0; pointIndex < target.points!.length; pointIndex++) {
      ...
      if (previous.points!.length - 1 >= pointIndex) {
        ...
        previousPoint = _DatumPoint<D>.from(targetPoint, targetPoint.x, lastPoint.y);
      }

      final x = ((targetPoint.x! - previousPoint.x!) * animationPercent) + previousPoint.x!;

      double? y;
      if (targetPoint.y != null && previousPoint.y != null) {
        y = ((targetPoint.y! - previousPoint.y!) * animationPercent) + previousPoint.y!;
    ...
    if (areaColor != null) {
      areaColor = getAnimatedColor(previous.areaColor!, target.areaColor!, animationPercent);
    }

    strokeWidthPx =
        ((target.strokeWidthPx - previous.strokeWidthPx) * animationPercent) + previous.strokeWidthPx;
  }
}
...
class _AnimatedLine<D> {
  ...
  void animateOut() {
    ...
    for (var index = 0; index < newTarget.points!.length; index++) {
      ...
      newPoints.add(
          _DatumPoint<D>.from(targetPoint, targetPoint.x, newTarget.measureAxisPosition!.roundToDouble()));
    }
  ...
  _LineRendererElement<D> getCurrentLine(double animationPercent) {
    ...
    _currentLine!.updateAnimationPercent(_previousLine!, _targetLine, animationPercent);
...
class _AreaRendererElement<D> {
  ...
  void updateAnimationPercent(
      _AreaRendererElement<D> previous, _AreaRendererElement<D> target, double animationPercent) {
    late _DatumPoint<D> lastPoint;
...
class _AnimatedArea<D> {
  ...
  _AreaRendererElement<D> getCurrentArea(double animationPercent) {
    ...
    _currentArea!.updateAnimationPercent(_previousArea!, _targetArea, animationPercent);
...
class _Range<D> {
  ...
  void includePoint(D? value) {
    if (value == null) {
      ...
      throw ArgumentError('Unsupported object type for LineRenderer domain value: '
          '${value.runtimeType}');
    }
  }
